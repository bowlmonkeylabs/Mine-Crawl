...
namespace BML.Scripts.MMFFeedbacks
{
    ...
    [FeedbackPath("GameObject/Safe Instantiate Object")]
    public class MMFSafeInstantiateObject : MMF_Feedback
    {
        ...
        /// use same parent as target
        [Tooltip("use same parent as target")]
        public bool UseTargetParent;
        /// the transform to parent the spawned object to
        [Tooltip("the transform to parent the spawned object to")]
        public Transform ParentTransform;
        /// randomize position offset
        [Tooltip("randomize position offset")]
        ...
        /// radius in which to choose random offset
        [Tooltip("radius in which to choose random offset")]
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            else
            {
                ...
                } else if(TargetTransform != null && UseTargetParent) {
                    parent = TargetTransform.parent;
                }

                _newGameObject = GameObjectUtils.SafeInstantiate(true, GameObjectToInstantiate, parent);
                if (_newGameObject != null)
                {
        ...
        protected virtual Vector3 GetPosition(Vector3 position)
        {
            ...
            if(UseRandomOffset) {
                ...
                posOffset = new Vector3(offset.x, .25f, offset.y) * OffsetRadius;
            }

            switch (PositionMode)
            {
                case PositionModes.FeedbackPosition:
                    return Owner.transform.position + posOffset;
                case PositionModes.Transform:
                    return TargetTransform.position + posOffset;
                case PositionModes.WorldPosition:
                    return TargetPosition + posOffset;
