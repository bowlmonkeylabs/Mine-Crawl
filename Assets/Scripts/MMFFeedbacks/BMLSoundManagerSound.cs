...
{
    /// <summary>
    /// This feedback will let you play a sound via the MMSoundManager with added features. You will need a game object in your scene with a MMSoundManager object on it for this to work.
    /// </summary>
    [ExecuteAlways]
    [AddComponentMenu("")]
    [FeedbackPath("Audio/BMLSoundManager Sound")]
    [FeedbackHelp("This feedback will let you play a sound via the MMSoundManager with added features. You will need a game object in your scene with a MMSoundManager object on it for this to work.")]
    public class BMLSoundManagerSound : MMF_Feedback
    {
        ...
        [Tooltip("whether or not this sound should play if the same sound clip is already playing X number of times")]
        public bool LimitConsecutivePlays = false;
        /// If LimitConsecutivePlays, how many can play at once
        [Tooltip("If LimitConsecutivePlays, how many can play at once")]
        public int MaxConsecutivePlays = 3;
        ...
        {
            if (LimitConsecutivePlays) 
            {
                if (BML_MMSoundManager.Instance.FindAllByClip(sfx).Count >= MaxConsecutivePlays)
                {
                    return;
                }
            }
