...
namespace BML.Scripts.MMFeedbacks
{
    /// <summary>
    ...
    [FeedbackPath("Transform/BML Destination")]
    public class BML_DestinationTransform : MMF_Feedback
    {
        ...
        public TransformSceneReference DestinationTransformSceneReference;
        public Transform Destination => DestinationTransformSceneReference.Value;
        ...
        protected virtual IEnumerator AnimateToDestination()
        {
            ...
            {
                _pointAPosition = ForceOrigin ? Origin.transform.localPosition : TargetTransform.localPosition;
                _pointBPosition = Destination.transform.localPosition;

                if (!AnimatePositionX) { _pointAPosition.x = TargetTransform.localPosition.x; _pointBPosition.x = _pointAPosition.x; }
                if (!AnimatePositionY) { _pointAPosition.y = TargetTransform.localPosition.y; _pointBPosition.y = _pointAPosition.y; }
                if (!AnimatePositionZ) { _pointAPosition.z = TargetTransform.localPosition.z; _pointBPosition.z = _pointAPosition.z; }
                
                _pointARotation = ForceOrigin ? Origin.transform.localRotation : TargetTransform.localRotation;
                _pointBRotation = Destination.transform.localRotation;
                
                if (!AnimateRotationX) { _pointARotation.x = TargetTransform.localRotation.x; _pointBRotation.x = _pointARotation.x; }
                if (!AnimateRotationY) { _pointARotation.y = TargetTransform.localRotation.y; _pointBRotation.y = _pointARotation.y; }
                if (!AnimateRotationZ) { _pointARotation.z = TargetTransform.localRotation.z; _pointBRotation.z = _pointARotation.z; }
                if (!AnimateRotationW) { _pointARotation.w = TargetTransform.localRotation.w; _pointBRotation.w = _pointARotation.w; }
                ...
                while ((journey >= 0) && (journey <= Duration) && (Duration > 0))
                {
                    ...
                    TargetTransform.localPosition = _newPosition;
                    TargetTransform.localRotation = _newRotation;
                    TargetTransform.localScale = _newScale;
                ...
                if (ForceDestinationOnEnd)
                {
                    if (NormalPlayDirection)
                    {
                        TargetTransform.localPosition = _pointBPosition;
                        TargetTransform.localRotation = _pointBRotation;
                        TargetTransform.localScale = _pointBScale;
                    }
                    else
                    {
                        TargetTransform.localPosition = _pointAPosition;
                        TargetTransform.localRotation = _pointARotation;
                        TargetTransform.localScale = _pointAScale;
                    }    
                }
                IsPlaying = false;
